...
public class RabbitConfig {
    ...
    TopicExchange topicExchange(){
        return new TopicExchange(RabbitConfig.EXCHANGE_B);
    }

    //把所有的队列都绑定到这个交换机上去
    @Bean
    Binding bindingExchangeA(Queue queueA, TopicExchange topicExchange) {
        return BindingBuilder.bind(queueA).to(topicExchange).with("routingKey.a");
    }
    @Bean
    ...
    Binding bindingExchangeC(Queue queueC,  TopicExchange topicExchange) {
        return BindingBuilder.bind(queueC).to(topicExchange).with("routingKey.#");
    }
}
